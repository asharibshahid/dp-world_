...
export type VesselLoadError = {
  file: string;
  message: string;
};

export type VesselLoadResult = {
  rows: VesselRow[];
  errors: VesselLoadError[];
};

const HEADER_TO_FIELD: Record<string, keyof VesselRow> = {
...
const DATE_FIELDS = new Set<keyof VesselRow>([
  "eta",
  "etd",
  "cutoffDate",
  "lastEsa",
...
const parseSheetRows = (sheet: xlsx.WorkSheet, terminal: string) => {
  ...
  return rows.slice(headerRowIndex + 1).reduce<VesselRow[]>((acc, row) => {
    const record: VesselRow = {
      vesselName: "",
      vesselType: "",
      voyageNo: "",
      rotation: "",
    ...
    const { terminal: _, ...fields } = record;
...
export const loadVessels = async (): Promise<VesselLoadResult> => {
  const baseDir = path.join(process.cwd(), "public");
  const allRows: VesselRow[] = [];
  const errors: VesselLoadError[] = [];
  ...
  if (isDev) {
    console.log(
      `[vessels] found files: ${files.length ? files.join(", ") : "none"}`
    );
  }

  for (const file of files) {
    const filePath = path.join(baseDir, file);
    const terminal = path.basename(file, path.extname(file)).toUpperCase();
    ...
    try {
      ...
      if (!stats.isFile() || stats.size === 0) {
        throw new Error("File is empty or not a file");
      }
      ...
      const rows = parseSheetRows(sheet, terminal);
      if (isDev) {
        console.log(`[vessels] ${file}: ${rows.length} rows`);
      }
      allRows.push(...rows);
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unknown error";
      if (isDev) {
        console.log(`[vessels] ${file}: failed (${message})`);
      }
  ...
  if (isDev) {
    console.log(`[vessels] total rows: ${allRows.length}`);
  }
